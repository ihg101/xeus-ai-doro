 ...
 *
 */
public class GMT_ExcelUtil {
	/**
	 * 컬럼정보, 테이블 정보 유효성을 검사한다.
	 *
	 * @param map
	 * @return 유효성 검사 통과 : "pass"
	 *         유효성 검사 불통과 : 에러메세지(String)
	 *
	 */
	...
	private static String validateTableInfo(HashMap<String, String> map) throws Exception {
	    ...
	    String excelRowCnt=map.get("excelRowCnt");
		...
		else if(!"pass".equals(validateExcelRowCnt(excelRowCnt))){
			msg=validateExcelRowCnt(excelRowCnt);
		}
	    return msg;
	}

	/*
	 * 테이블 이름 유효성 검사(특수문자 사용x, 널 체크)
	 */
	...
	/*
	 * 컬럼 개수 유효성 검사(위도,경도 외 1개 이상 컬럼 필요)
	 */
	 ...
	 * 엑셀 row 수 유효성 검사
	 */
	private static String validateExcelRowCnt(String excelRowCnt) {
		String msg="pass";

		if(excelRowCnt==null || "".equals(excelRowCnt)){
			msg="엑셀 줄 수를 입력해주세요.";
		}
