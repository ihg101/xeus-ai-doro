...
public class CodeConvertor {
	...
	private ArrayList<CodeVo> cde;
	private int cdeSize;

	public CodeConvertor(ArrayList<CodeVo> cde) {
		this.cde = cde;
	 ...
	 * @return str - 코드명
	 */
	public String convertNameToCdeInGrp(String group, String name){
		String str = "";

		if(!"".equals(name) && name != null && !"".equals(group) && group != null){
			for(int i=0; i<cdeSize; i++){
				if(cde.get(i).getGrpCde().trim().equals(group.trim()) && cde.get(i).getCdeNm().trim().equals(name.trim())){
					str = cde.get(i).getCdeCde().trim();
				}
			}
		}else{
