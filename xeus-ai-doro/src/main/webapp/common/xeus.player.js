...
XeusGate.Player = function() {
    ...
    var Player = function(options) {
        ...
        document.addEventListener("visibilitychange", function() {
            if (document.visibilityState === 'hidden') {
                ...
                if (_self.options.videoElement) {
                    _self.options.videoElement.pause();
                    if (_self.options.debug) {
                        console.log("player hidden.... paused...");
                    }
                }
            } else {
                _self.playerVisible = true;
                if (_self.options.videoElement) {
                    _self.options.videoElement.play();
                    if (_self.options.debug) {
                        console.log("player visible.... playing...");
                    }
                }
            }
        });
        ...
        this.renderer = new XeusGate.Renderer.MPEG4(this, this.options);
...
XeusGate.Source = function() {
    ...
    WsSource.prototype.onOpen = function() {
        ...
        var cmd = "{" +
            ...
            "}";
        if (this.options.debug) {
            console.log("WebSocket Open: " + cmd);
        }
    ...
    WsSource.prototype.onClose = function() {
        if (this.options.debug) {
            console.log("WsSource.prototype.onClose...");
        }
        this.playing = false;
        this.retryCount = 0;
        //
    ...
    WsSource.prototype.destroy = function() {
        ...
        if (this.options.debug) {
            console.log("WebSocket Close: " + cmd);
        }
        clearInterval(this.connectIntervalId); // 연결시간 초과 알림
        // clearTimeout(this.reconnectTimeoutId); // 연결재시도..
        this.retryCount = 0;
        ...
        this.socket.close();
        if (this.options.debug) {
            console.log("WsSource.prototype.destroy...");
        }
    };
...
XeusGate.Renderer.MPEG4 = function() {
    "user strict";

    var VideoRenderer = function(player, options) {
    ...
    VideoRenderer.prototype.log = function(msg) {
        if (this.options.debug) {
            console.log(msg);
        }
    };

    VideoRenderer.prototype.appendByteArray = function(buffer1, buffer2) {
    ...
    VideoRenderer.prototype.start = function() {
        window.MediaSource = window.MediaSource || window.WebKitMediaSource;
        var isMSESupported = !!window.MediaSource;
        if (!isMSESupported) {
            if (this.options.debug) {
                console.log('Oops! Browser does not support media source extension.');
            }
            this.player.destroy();
            return;
        }
    ...
    VideoRenderer.prototype.onMSEOpen = function() {
        this.mediaSourceReady = true;
    };

    VideoRenderer.prototype.onMSEClose = function() {
        if (this.options.debug) {
            console.log("VideoRenderer.prototype.onMSEClose");
        }
        this.player.destroy();
    };

    VideoRenderer.prototype.cleanUpBuffer = function() {
        if (this.sourceBuffer.buffered && this.sourceBuffer.buffered.length) {
            for (var i = 0; i < this.sourceBuffer.buffered.length; ++i) {
                ...
                if ((start < endOffset) && !this.sourceBuffer.updating) {
                    ...
                    if (this.options.debug) {
                        console.log("cleanUp cur: " + cur + " off: " + this.lastCleanUpOffset + " end: " + end);
                    }
    ...
    VideoRenderer.prototype.feed = function(data) {
        ...
        if (this.initialized && !this.sourceBuffer.updating) {
            ...
            if (this.sequence % offset == 0) {
                try {
                    ...
                    if (this.options.debug) {
                        console.log(e.name + " : sourceBuffer.appendBuffer ");
                    }
                }
            }
        }
    ...
    VideoRenderer.prototype.destroy = function() {
        ...
        this.queue = null;
        if (this.options.debug) {
            console.log("VideoRenderer.prototype.destroy...");
        }
    };
...
// 아래 미 구현.... 구현 예정...
/////////////////////////////////////////////////
XeusGate.Renderer.Canvas = function() {
    "user strict";
    ...
    CanvasRenderer.prototype.feed = function(data) {
