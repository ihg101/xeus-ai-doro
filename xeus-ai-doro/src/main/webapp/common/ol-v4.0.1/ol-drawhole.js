...
{	var map = this.getMap();
	...
	var current = null;
	if (features)
	{	if (features.getGeometry().getType() !== "Polygon") current = null;
		else if (features.getGeometry().intersectsCoordinate(coord)) current = features;
		else current = null;
	}
	else current = null;
...
{	var c = e.feature.getGeometry().getCoordinates()[0];
	...
	var drawWkt = format.writeGeometry(this.getPolygon().getGeometry());
	$("#bigdataView").find("#columnTable").find("tbody").find("tr:eq(" + trIdx + ")").attr("wkt", drawWkt);
	$("#bigdataView").find("#geometryTable").find("tbody").find("tr:eq(" + trIdx + ")").find("input[type=hidden]").val(drawWkt);
