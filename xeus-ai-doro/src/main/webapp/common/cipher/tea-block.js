 ...
 * 
 * @param {string} plaintext String to be encrypted (multi-byte safe) @param
 * {string} password Password to be used for encryption (1st 16 chars) @returns
...
Tea.encrypt = function(plaintext, password) {
	...
	// convert string to array of longs after converting any multi-byte chars to
	// UTF-8
 ...
 * 
 * @param {string} ciphertext String to be decrypted @param {string} password
 * Password to be used for decryption (1st 16 chars) @returns {string} decrypted
 * text
 */
...
Tea.strToLongs = function(s) { // convert string to array of longs, each
	// containing 4 chars
	// note chars must be within ISO-8859-1 (with Unicode code-point < 256) to
	// fit 4/long
...
Tea.longsToStr = function(l) { // convert array of longs back to string
	...
	return a.join(''); // use Array.join() rather than repeated string appends
	// for efficiency in IE
}
