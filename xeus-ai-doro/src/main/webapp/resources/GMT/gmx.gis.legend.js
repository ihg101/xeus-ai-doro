	...
	createTMSLayer : function(){
		...
		if("GMXMAP" in window){
			...
			for(var i=0; i<layerList.length; i++){
				var tms = layerList[i];
				var data = GMXLAYER.LayerList[tms.get("id")];
				...
				if(geomType === "T"){
					...
					var $edit = $("<button>").addClass("edit").text("설정").data(data).click(function(){
						GMXLAYER.createLayerInfoDialog($(this).data());
					});

					var $copyTd1 = $tdTemp.clone().width(15).height(30);
					var $copyTd2 = $tdTemp.clone().width(35);
					var $copyTd3 = $tdTemp.clone().attr("colspan", "3");
					var $copyTd4 = $tdTemp.clone().addClass("tRight");
...
//						수정 권한이 있는 레이어
	...
	createThemeLayer : function(data){
		...
		for(var i=0; i<data.theme.length; i++){
			...
			var $check = $("<input type='checkbox'>").addClass("toggleThemeLayer").data(data).attr("id", data.layer.tblId+":"+thmStartVal).attr("c", thmFieldId).attr("t", thmFieldTyp).attr("sv", thmStartVal).attr("ev", thmEndVal).prop("checked", true);
			var $checkC = $("<label class='checkboxC'></label>").attr("for", data.layer.tblId+":"+thmStartVal);
...
//			GMXMAP.setLayerVisible(layerNm, visible);
		...
		$themeWrap.find(".toggleThemeLayer").change(function(){
			...
			if(thmFieldTyp === "N"){
				...
				if(lyrNm === "굴삭기"){
					var type = sessionStorage.getItem('NOW_MENU') === "btn-monitor-view" ? 'LIST' : null;
					var a = {type :type};
					filter = GMXLAYER.customFilters(a, kv); 
					WIDGET.getPastEventListWidget(kv);
				} else{
