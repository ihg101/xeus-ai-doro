(function(){
	...
	function callView(offset, limit){
		...
		_common.callAjax("/cctv/getCctvList.json", _param, function(json){
			//TODO 레이어
			//$('.tab[target=eventView]').data('cctv').reload();
			...
			if(json.result.length == 0){
				...
				for(var i=0; i<json.result.length; i++){
					var label = json.result[i].cctvNm;
					if(!_common.utils.isNullAndEmpty(json.result[i].cctvNm)){
					...
					label = _common.utils.validNull(json.result[i].cctvNm);
	...
	$(".contentWrapper").find("#excelBtn").click(function(){
		if($(".contentWrapper").find("#resultTable").find("tbody").children('.search_result').length == 0){
			...
			if(confirm("검색결과를 엑셀로 다운로드 하시겠습니까?")){
				...
				_common.postForm.submit("/cctv/getCctvListExcel.do", excelParam);
			}
		}
	});
