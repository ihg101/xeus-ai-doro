...
//				if(localStorage) localStorage[list[i].get("id")] = toggleVal;
	...
	/**
	 * 현재 Map에 있는 Source 객체에서 key Value에 대응하는 faeutre 리스트를 반환
	 *
	 *
	 */
	...
	GMXMAP.getFeaturesByKeyValueAtMap = function(idOrNm,key, value){
		...
		if(idOrNm == undefined  || key == undefined || value == undefined){
			return [];
		}
	...
	/**
	 * 현재 Map에 있는 Source 객체에서 Primary key(_gid)에 대응하는 단일 feature 반환
	 *
	 */

	GMXMAP.getFeatureByPkValueAtMap = function(idOrNm,value){

		if(idOrNm == undefined || value == undefined){
			return null;
		}
	...
	/**
	 * DB에 있는 공간테이블에서 key value에 대응하는 feature 리스트를 반환
	 *
	 */
...
//				_source.clear();
	...
	/**
	 * DB에 있는 공간테이블에서 고유 key(_gid)에 대응하는 단일 feature 반환
	 *
	 */
...
//				_source.clear();
	...
	GMXMAP.setLayerVisible("kais_emd_as",true);
	GMXMAP.setLayerVisible("asset_statistics_sig",true);
	GMXMAP.setLayerVisible("daum_map",true);
	...
	GMXMAP.getLayer("kais_emd_as").setZIndex(999);
	 /*
	...
	GMXMAP.selectResultFt = new ol.interaction.Select({
		layers: function(l){
			return l.get('selectable') == true;
		}
	});

	GMXMAP.addInteraction(GMXMAP.selectResultFt);

	GMXMAP.on("click", function(e){
		var layer = null;
		var emdData = null;
		var feature = null;
		GMXMAP.selectResultFt.getFeatures().clear();
		feature = GMXMAP.forEachFeatureAtPixel(e.pixel, function (ft, l) {
			if(l.get("id") == "kais_emd_as"){
				l.set('selectable', true);
				GMXMAP.selectResultFt.on('select', function(e){

					var _Feature = e.selected[0];

					var _Data = GMXLAYER.LayerList["kais_emd_as"];
...
//							})
		...
		if (feature != null){
			$(".graph_select_wrap").find(".emdCd").val(feature.getProperties().emd_cd).prop("selected", true);
			$(".btn_search").click();
		}
	});
});