...
var xeusJsonParser = {
	...
	Start : function(_Map){
		...
		WIDGET.getEventListWidget($("#eventList"));
	...
	Stop : function(){
		...
		if(xeusJsonParser.getJson().uSvcOutbId == uSvcOutbId){
			...
			if(GMXMAP["eventVectorLayer"].getSource()) GMXMAP["eventVectorLayer"].getSource().clear();
	...
	setEventContent : function(json){
		...
		if(_Json.statEvetTypCd != "MBS"){
			...
			$("#ctntTable").find("#outbPosX").text(_Json.outbPos[0].x);
			$("#ctntTable").find("#outbPosY").text(_Json.outbPos[0].y);
