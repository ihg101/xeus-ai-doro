...
var SessionWS = function(_Map){
	...
	this.request = function(e){
		...
		if(("code" in _thisJson) && (_thisJson.code === "GMX-1002")){
			window.location = context + "/user/signOutByAdmin.do?action=signOutNow&adminNotice=다른 PC에서 로그인하여 자동 로그아웃 되었습니다.";
			return;
		}
		...
		if("AdminNotice" in _thisJson){
			...
			if(user.contains(userId)){
				if(action == "signOutNow"){
					window.location = context + "/user/signOutByAdmin.do?action=signOutNow&adminNotice=" + msg;
				...
				if(action == "signOut"){
					window.location = context + "/user/signOut.do";
				}
			}
		}
	}
