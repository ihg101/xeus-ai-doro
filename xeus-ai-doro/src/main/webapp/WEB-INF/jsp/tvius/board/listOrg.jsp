...
<%
	...
	String columnName[] = {"구분", "총계", "1월", "2월", "3월", "4월", "5월", "6월", "7월",
							"8월", "9월", "10월", "11월", "12월"};
	String baseColor[] = {"", "220,220,220", "151,187,205", "240,73,73", "151,220,151", "151,220,187",
...
<div id="searchBox" class="customScroll"
	...
	<div class="contentWrapper statTable customScroll hidden" style="width: 100%;">

		<h3 class="title">기관별 영상반출 현황</h3>
		<div class="box_style">
			<div class="info_box wd100">
				<div>
					<span>신청유형</span> <select id="reqGbnCd" name="reqGbnCd" class="sendData" name="reqGbnCd" style="width: 100px;">
				...
				<div>
					<span> 검색년도</span> <select id="serch_year" style="width: 100px;">
					</select>
					<span style="color: #666"> * 선택한 신청유형과 검색년도 별로 조회할 수 있습니다.</span>
					<button id="btn_sch" class="btn_style2">조 회</button>
				</div>
				<button id="xls_down_btn" class="btn_Dstyle" key="list_use">
					엑셀내려받기
				</button>
			</div>
		</div>
		<table id="listuse" border="0" cellspacing="0">
			<tr>
				<%
			...
			<tr>
				<%
				    for (Map.Entry<String, String> et : rows.get(i).entrySet()) {
									...
									if (j > 1) {
										charTmp[j - 2] = et.getValue();
									}
									if ("구분".equals(et.getKey()))
										chartTitle[i] = et.getValue();
									j++;
				%>
			...
			<%
			    if (i < rows.size() + 1)
								chartStr[i] = StringUtils.join(charTmp, ",");
	...
	%>
	<div class="contentWrapper statChart customScroll">
		<!-- height:50%; -->
		<%
		    } else {
		%>
			...
			<div id="chart_title" style="margin: 15px 0 10px 0">
				<ul>
					...
					<li class="stat_list info_box">
						<div class="stat_list_color" style="background: rgba(<%=colorArr[i]%>,0.5);"></div> <span class="stat_list_title"
						style="margin-right: 10px;"><%=chartTitle[i]%></span>
					</li>
					<%
								}
					%>
