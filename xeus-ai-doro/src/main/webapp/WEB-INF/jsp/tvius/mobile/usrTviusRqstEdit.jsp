					...
					<tr>
						...
						<td>
							...
							<span class="ui-icon-plus ui-btn-icon-right ui-shadow ui-btn ui-corner-all fileinput-button" data-role="button" data-icon="plus">
								<span>파일선택</span>
			                    <input type="file" name="uploadImg" id="uploadImg" data-role="none" accept=".pdf, .txt, .hwp, .xls, .xlsx, .ppt, .rtf, .doc, .jpg, .jpeg, .png, .zip, .7z">
							</span>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							<select class="sendData" id="videoSmy" name="videoSmy" required>
								<option value="N" <% if("N".equals(vo.getVideoSmy())) out.print("selected"); %>>미신청</option>
								<option value="Y" <% if("Y".equals(vo.getVideoSmy())) out.print("selected"); %>>신청</option>
							</select>
						</td>
					</tr>
					<tr>
...
<% for(int i=0; i<avi.size(); i++){ %>
								<li>
									<fieldset class='ui-grid-a'>
										<button type="button" class="selectCctvInfo ui-btn ui-icon-delete ui-btn-icon-right" data-role="button" cctvNm="<%= avi.get(i).getCctvLabel() %>" mgrNo="<%= avi.get(i).getCctvMgrNo() %>"><%= avi.get(i).getCctvLabel() %></button>
										<div class="ui-block-a"><p>시작시간</p><input type="datetime-local" class="sDate sendData" value="<%= DateUtil.formatDate(avi.get(i).getSecStDat()).replace(" ", "T") %>" placeholder="선택" required></div>
										<div class="ui-block-b"><p>종료시간</p><input type="datetime-local" class="eDate sendData" value="<%= DateUtil.formatDate(avi.get(i).getSecEdDat()).replace(" ", "T") %>" placeholder="선택" required></div>
									</fieldset>
								</li>
<% } %>
							</ul>
					...
					<tr>
						<td colspan="4">
							<button type="button" data-theme="a" id="editBtn">영상 반출 재신청</button>
						</td>
					</tr>
				</tbody>
			</table>
